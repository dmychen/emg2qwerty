...
The model performs **sequence-to-sequence transcription from surface EMG (sEMG) signals to typed characters**. Two wristband devices (left and right, 16 differential electrode pairs each) record muscle electrical activity at 2kHz while a user types on a keyboard. The sEMG signal is generated by **motor units** — a motor neuron and the muscle fibers it innervates — firing electrical pulses that superimpose additively at the skin surface. This is fundamentally different from speech (which is a time-varying filter applied to a source); sEMG is a purely additive combination of muscle activations. Notably, sEMG activity typically **precedes** the physical keypress by tens of milliseconds, as the electrical signal occurs during muscle fiber stimulation before the resulting motion.

This is framed as a **temporal classification** problem analogous to Automatic Speech Recognition (ASR): a fixed-rate continuous signal is transduced into a variable-length character sequence, without explicit alignment between signal frames and characters. The character set has 96 classes (a-z, A-Z, 0-9, punctuation, space, enter, backspace, shift) plus a CTC blank token (97 total). Evaluation uses Character Error Rate (CER) computed via Levenshtein edit distance.

A key challenge is **domain shift** at three levels: (1) cross-user variation from anatomy/physiology differences, (2) cross-session variation from differences in band placement when re-donning, and (3) behavioral variation from different typing strategies (finger-to-key mapping, force level). The generic (unpersonalized) model achieves >50% CER on unseen users; personalized finetuned models reach ~7% CER with a language model. A second challenge is **co-articulation**: the sEMG pattern for a keystroke is heavily influenced by the characters typed immediately before and after it, requiring models with receptive fields long enough to capture bigram/trigram context (~1 second).

### 1.2 CNN Model Architecture (TDS-Conv)

The baseline model is a **Time-Depth Separable (TDS) Convolutional** encoder, from the paper *"Sequence-to-Sequence Speech Recognition with Time-Depth Separable Convolutions"* (Hannun et al., 2019). The core idea of TDS is **decoupling temporal aggregation from channel mixing**: temporal convolutions use small channel counts (`c`) with `k x 1` kernels (only `kc²` parameters), followed by fully-connected layers that mix across channels. This allows large receptive fields with far fewer parameters than standard convolutions or RNNs (the TDS paper reports 37M params vs 136M for an RNN baseline with better accuracy). The parameter efficiency of TDS was specifically chosen for emg2qwerty because a wide receptive field (~1 second / 125 spectrogram frames) is needed to capture co-articulation context without learning an implicit language model. The full pipeline:
...
The 4 TDS blocks each have kernel width 32, giving a total temporal receptive field of ~125 spectrogram frames (1 second). The convolutions are causal-like (no padding), so the output sequence is shorter than the input by this receptive field. The TDS paper shows a sharp increase in WER when receptive field drops below a critical threshold — this is equally important for emg2qwerty due to co-articulation.
